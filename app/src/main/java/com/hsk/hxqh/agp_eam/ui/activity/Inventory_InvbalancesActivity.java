...
import com.hsk.hxqh.agp_eam.adpter.InvbalancesAdapter;
 ...
 */
public class Inventory_InvbalancesActivity extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener, SwipeRefreshLayout.OnLoadListener {
    ...
    private InvbalancesAdapter invbalancesAdapter;
    private SwipeRefreshLayout refresh_layout = null;
    private int page = 1;
    public INVENTORY inventory;
    public ArrayList<INVBALANCES> assetArrayList = new ArrayList<>();
...
//        confirmBtn = (Button) findViewById(R.id.confirm);
    ...
    protected void initView() {
        ...
        titleTextView.setText(getResources().getString(R.string.invbalances));
...
//        confirmBtn.setOnClickListener(confirmBtnOnClickListener);
        layoutManager = new LinearLayoutManager(Inventory_InvbalancesActivity.this);
...
//        mBasOut = new SlideBottomExit();
    ...
    private void getData() {
        HttpManager.getDataPagingInfo(Inventory_InvbalancesActivity.this, HttpManager.getInvbalancesUrl(inventory.getITEMNUM(),inventory.getLOCATION(),inventory.getSITEID(),inventory.getITEMSETID(),1, 20), new HttpRequestHandler<Results>() {
            @Override
            ...
            public void onSuccess(Results results, int totalPages, int currentPage) {
                ArrayList<INVBALANCES> item = JsonUtils.parsingINVBALANCES(Inventory_InvbalancesActivity.this, results.getResultlist());
                ...
                if (item == null || item.isEmpty()) {
                    ...
                    if (item != null || item.size() != 0) {
                        if (page == 1) {
                            assetArrayList = new ArrayList<INVBALANCES>();
    ...
    private void initAdapter(final List<INVBALANCES> list) {
        invbalancesAdapter = new InvbalancesAdapter(Inventory_InvbalancesActivity.this, R.layout.list_item_invbalances, list);
        recyclerView.setAdapter(invbalancesAdapter);
        invbalancesAdapter.setOnRecyclerViewItemClickListener(new BaseQuickAdapter.OnRecyclerViewItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
                Intent intent = new Intent(Inventory_InvbalancesActivity.this, InvbalancesDetailsActivity.class);
                Bundle bundle = new Bundle();
                bundle.putSerializable("invbalances", assetArrayList.get(position));
    ...
    private void addData(final List<INVBALANCES> list) {
        invbalancesAdapter.addData(list);
    }
